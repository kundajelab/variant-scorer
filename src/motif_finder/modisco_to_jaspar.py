        ...
        for pattern_name, pattern in sorted(metacluster.items(), key=key):
            ...
            if len(trimmed) > args.min_length:
                ...
                if num_seqlets >= args.min_seqlets:
                    ...
                    curr_motif = "NoMatch--" + name + "." + pattern_name if (type(curr_motif) == float and np.isnan(curr_motif)) else curr_motif
                    np.savetxt(args.output_dir + '/' + curr_motif + '.ppm.txt', trimmed.transpose())
